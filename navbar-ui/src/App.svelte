<script>
	...
	const activeLinkFollower = (path) => {
		const topLevelPath = path.split('/').find((p) => p !== '')
		const link = document.querySelector(`#navbar a[href^="/${topLevelPath}"]`)
		const { left, width } = link 
			? link.parentElement.getBoundingClientRect()
			: {left: 0, width: 0};
			
		followerStyle = `width: ${width}px; left: ${left}px;`
	}

	onMount(() => {
		activeLinkFollower(window.location.pathname)
		const pushStateListener = window.addEventListener('pushState', function(e) {
			currentUrl = window.location.pathname
			activeLinkFollower(currentUrl)
		})
...
#follower {
	...
	padding-bottom: 3px;
	position: absolute;
	transition: width .5s ease-in-out, left .25s ease-in-out;
}
</style>

<nav>
	<ul id="navbar" use:links>
		<li>
